...
opreat      = sys.argv[1];
opreats    = ['join','left','check','send','getusers','adduser','getUsers','joining','CX','MovePro','R'];
...
async def Controll():
	try:
		...
		if opreat == 'R':
			sart = await app.get_messages(chat_id=str(sys.argv[3]), message_ids=int(sys.argv[4]));
			print(sart);
		if opreat == 'check':
			try:
				await app.get_me();
	...
	if opreat == 'getUsers':
		...
		try:
			...
			async for user in getUser:
				if user.user.username is not None:
					groupUsers.append(f"@{user.user.username}");
				else:
