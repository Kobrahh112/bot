...
opreat      = sys.argv[1];
opreats    = ['join','left','check','send','getusers','adduser','getUsers','joining','CX','MovePro','View','sos','getData'];
...
async def Controll():
	...
	if opreat == 'getData':
		chUserName    = str(sys.argv[3]);
		mgID                  = int(sys.argv[4]);
		textSearch         = str(sys.argv[5]);
		getMessage     = await app.invoke(functions.channels.GetMessages(
			channel=await app.resolve_peer(chUserName),
			id=types.InputMessageID(id=mgID)
		));
		#getMessage     = await app.get_messages(
			#chat_id=chUserName,
			#message_ids=mgID
		#);
		...
		async for Message in getMessage:
			...
			if Message.reply_markup is not None:
				if Message.reply_markup.inline_keyboard is not None:
					KeyboardInfo    = Message.reply_markup.inline_keyboard;
					async for Button in KeyboardInfo:
						if Button.text == textSearch:
							cbData   = Button.callback_data;
							print('true');
							print(cbData);
						else :
							print(Button.text);
	...
	if opreat == 'sos':
		groupID   = str(sys.argv[3]);
		offset      = int(sys.argv[4]);
		limit         = int(sys.argv[5]);
		xxd = 0;
		try :
			xxm          = app.get_chat_members(chat_id=groupID,limit=limit, query="");
			print('Yesss');
			async for hj in xxm:
				try :
					if hj.user.id is not None:
						xxd += 1;
						print(xxd);
				except FloodWait as d:
					print('nbbb');
					#async def oki(d):
						#await async.sleep(d.value);
